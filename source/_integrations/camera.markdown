...
카메라가 이를 지원하고, [`stream`](/integrations/stream) 연동이 설정된 경우, 카메라를 프런트 엔드와 지원되는 미디어 플레이어에서 스트리밍할 수 있습니다.
...
| Service data attribute | Optional | Description |
| ---------------------- | -------- | ----------- |
| `entity_id`            |     yes  | 모션 감지를 비활성화할 엔티티의 이름  (예: `camera.living_room_camera`.) |
...
| Service data attribute | Optional | Description |
| ---------------------- | -------- | ----------- |
| `entity_id`            |      no  | 예를 들어 `camera.living_room_camera`에서 스냅샷을 생성할 엔티티의 이름. |
| `filename`             |      no  | 파일이름의 템플릿. 변수는 `entity_id`, 예: {% raw %}`/tmp/snapshot_{{ entity_id }}`{% endraw %}. |
