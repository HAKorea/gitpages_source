...
Philips Hue support는 조명과 센서 플랫폼을 구동할 수 있는 허브로 Home Assistant에 통합되어 있습니다. Philips Hue 플랫폼을 설정하는 기본 방법은 [discovery component](/integrations/discovery/)를 활성화하는 것입니다.
...
Hue 플랫폼에는 조명 그룹의 색상을 한번에 설정하기위한 고유한 씬 개념이 있습니다. Hue 장면은 매우 저렴하고 모든 종류의 앱에서 만들 수 있으며 (동시에 2개이상의 조명을 변경하는 유일한 방법이므로) 거의 삭제되지 않습니다. 전형적인 Hue 허브에는 수백개의 씬이 저장되어 있을겁니다.

사용자 인터페이스 과부하를 피하기 위해 장면을 직접 노출시키지 않습니다. 대신 Hue가 있습니다. hue_activate_scene 서비스는 `automation` 또는 `script` 구성요소에서 사용할 수 있습니다. 이렇게 하면 Home Assistant에서 표준 씬을 사용할 때와 같이 한 번에 하나씩이 아니라 모든 전구가 한 번에 전환됩니다.
...
| ---------------------- | -------- | --------------------------------------------------------------------- |
| `group_name`           | no       | 라이트의 그룹/방 이름입니다. 공식 Hue 앱에서 찾으십시오. |
| `scene_name`           | no       | 장면의 이름. 공식 Hue 앱에서 찾으십시오.            |

_Note_: `group_name`은 홈어시스턴트 그룹 이름에 대한 참조가 아닙니다. Hue 앱에서 그룹/룸의 이름만 될 수 있습니다.

### 그룹과 씬 이름 찾기

이 이름들을 어떻게 찾습니까?

가장 쉬운 방법은 방(그룹)과 씬 이름으로 구성된 2 세대 Hue 앱의 씬만 사용하는 것입니다. 앱에 표시되는 방 이름과 씬 이름 값을 사용하십시오. Home Assistant 인스턴스의 `dev-service` 콘솔을 사용하여 이러한 작업이 작동하는지 테스트할 수 있습니다.

또는 이 [gist](https://gist.github.com/sdague/5479b632e0fce931951c0636c39a9578)를 사용하여 모든 방과 씬 이름을 가져올 수 있습니다. 이는 어떤 그룹과 씬이 함께 작동하는지 알려주지는 **않지만** `dev-service` 콘솔에서 테스트할 수 있는 값을 얻는 것으로 충분합니다.

### 경고 (Caveats)

Hue API는 씬을 직접 활성화하지 않습니다. 오히려 Hue 그룹(일반적으로 방, 특히 2 세대 앱을 사용하는 경우)과 연결되어야 합니다. 그러나 Hue 씬은 실제로 그룹을 참조하지 않습니다. 휴리스틱 매칭(heuristic matching)이 사용됩니다.

Hue에서 그룹 이름이나 씬 이름이 고유하지는 않습니다. 홈어시스턴트에서 Hue 씬을 호출하여 예기치 않은 동작이 관찰되는 경우 Hue 앱에서 Hue 씬의 이름을 보다 구체적으로 지정하십시오.

Hue 허브에는 씬에 대한 공간이 제한되어 있으며 새 씬이 만들어지면 해당 공간을 오버플로하는 씬을 삭제합니다. API 문서에 따르면 이것이 "최근에 사용된" 씬을 기반으로합니다.