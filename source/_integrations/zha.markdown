...
## ZHA 예외 및 편차 처리 (ZHA exception and deviation handling)
...
성공(Success) 대화 상자가 나타나거나 팝업에 오류가 표시됩니다. 홈어시스턴트가 USB 장치에 액세스 할 수 없거나 장치가 최신 상태가 아닌 경우 오류가 발생했을 수 있습니다 (문제 해결 참조).
